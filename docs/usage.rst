...
.. js:function:: $.validate(url, options)
    ...
    * `fieldSuccessCallback` is a callback function that is triggered when the field is valid. It takes one parameter the field that has been validated successfully.  In the the example it adds an icon after the field. This can be used to add some extra information to the form that the field has been valid. For example add a green border around the field or a message saying the e-mail address is still available.
    * `formSuccessCallback` is a callback function that is triggered when the form is valid. After it has been submited. This callback should be implemented to for example redirect to user to a success page or just remove the form and show a message the form has been submited successfully.
    * `fieldInvalidCallback` is a callback function that is triggered when a field is invalid. You could use this to remove style/elements you've added if the field has been marked as valid before. In other words if fieldSuccessCallback has been called on the field and added some extra markup/style.
